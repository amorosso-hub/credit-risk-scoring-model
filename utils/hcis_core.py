...
def hcis_band(score: float, t_low: float, t_high: float) -> str:
        # 숫자로 변환할 수 없는 값은 판단하지 않음
    try:
        score = float(score)
    except (TypeError, ValueError):
        return "판단불가"

    # NaN, 양의 무한대, 음의 무한대 차단
...
def compute_hcis_columns(
    ...
    *,
    ...
    # 숫자로 바꿀 수 없는 값은 NaN으로 처리
    pd_series = pd.to_numeric(out[pd_col], errors="coerce")

    # 부도확률은 유한한 숫자이며 0~1 범위여야 함
    valid_pd = (
        pd_series.notna()
        & np.isfinite(pd_series)
        & pd_series.between(0, 1)
    )

    # 잘못된 값은 NaN으로 유지
    pd_series = pd_series.where(valid_pd, np.nan)

    # 정상 범위 값에만 계산용 상·하한 적용
